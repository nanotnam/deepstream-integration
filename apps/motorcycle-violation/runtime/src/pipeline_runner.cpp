...
#include "motorcycle_violation/gpu_inference_executor.hpp"
...
std::filesystem::path plugin_root() {
  const char* configured = std::getenv("MOTORCYCLE_VIOLATION_PLUGIN_ROOT");
...
std::filesystem::path bundle_root() {
  const char* configured = std::getenv("MOTORCYCLE_VIOLATION_BUNDLE_ROOT");
  return configured != nullptr && *configured != '\0'
             ? std::filesystem::path(configured)
             : std::filesystem::path("/opt/mbfs/share/motorcycle-violation/models/bundles");
}
...
bool write_vehicle_config(const PipelineConfig& config, const EngineSet& engines,
  ...
  const std::filesystem::path temporary =
      ...
      ("motorcycle-violation-vehicle-" + std::to_string(static_cast<unsigned long>(getpid())) +
       ".txt");
...
class PipelineRunner::Implementation {
 public:
  ...
  int run(std::string* error) {
    ...
    pipeline_ = gst_pipeline_new("motorcycle-violation");
