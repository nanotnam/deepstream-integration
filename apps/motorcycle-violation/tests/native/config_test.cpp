...
#include "motorcycle_violation/engine_contract.hpp"
...
int main() {
  ...
  assert(motorcycle_violation::load_pipeline_config(
      root / "apps/motorcycle-violation/configs/file.yaml", &config, &error));
  assert(config.schema == "mbfs.deepstream-pipeline/v1");
  assert(config.source.type == motorcycle_violation::SourceType::kFile);
  ...
  assert(motorcycle_violation::pipeline_description(config).find("nvinfer[plate,batch=1]") !=
         std::string::npos);
  assert(motorcycle_violation::pipeline_description(config).find("every-vehicle-roi") !=
         std::string::npos);
  assert(motorcycle_violation::pipeline_description(config).find(
             "app-kafka-publisher[nvds_msgapi]") != std::string::npos);
  assert(motorcycle_violation::pipeline_description(config).find("nvmsgconv") ==
         std::string::npos);
  assert(motorcycle_violation::queue_overflow_name(config.processing.queue_overflow) ==
         "block");

  std::string uri;
  assert(motorcycle_violation::resolve_source_uri(
      config, [](const char*) { return "file:///tmp/test.mp4"; }, &uri, &error));
  assert(uri == "file:///tmp/test.mp4");
  assert(motorcycle_violation::redact_uri("rtsp://user:pass@camera/live") ==
  ...
  assert(!motorcycle_violation::validate_pipeline_config(invalid_processing, &error));
  ...
  const motorcycle_violation::EngineBatchProfiles static_profiles;
  assert(motorcycle_violation::validate_engine_batch_profiles(
  ...
  assert(!motorcycle_violation::load_pipeline_config(invalid, &config, &error));
  ...
  assert(!motorcycle_violation::load_pipeline_config(legacy, &config, &error));
