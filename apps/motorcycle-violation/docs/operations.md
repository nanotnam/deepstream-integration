...
profile. `MOTORCYCLE_VIOLATION_SOURCE_URI` accepts a file URI or RTSP URI according to the selected
profile. `MOTORCYCLE_VIOLATION_KAFKA_BROKERS` contains the Kafka connection string expected by the
DeepStream adapter.
...
```bash
docker build -f apps/motorcycle-violation/Dockerfile -t motorcycle-violation:local .
docker run --rm motorcycle-violation:local \
  --config /opt/mbfs/share/motorcycle-violation/configs/file.yaml --validate-only
```
...
container installs profiles under `/opt/mbfs/share/motorcycle-violation/configs`.
...
docker run --rm --gpus all \
  ...
  -e MOTORCYCLE_VIOLATION_SOURCE_URI=file:///data/test.mp4 \
  -e 'MOTORCYCLE_VIOLATION_KAFKA_BROKERS=kafka;29092' \
  -v "$PWD/.local/engines:/opt/mbfs/.local/engines:ro" \
  -v "$PWD/.local/test-video:/data:ro" \
  motorcycle-violation:local \
  --config /opt/mbfs/share/motorcycle-violation/configs/file.yaml
```
...
```bash
docker run --rm motorcycle-violation:local \
  --config /opt/mbfs/share/motorcycle-violation/configs/rtsp.yaml --print-gst-graph
```
