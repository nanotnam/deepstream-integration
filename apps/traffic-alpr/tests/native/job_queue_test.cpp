...
class TestFrameResource final : public traffic_alpr::FrameResource {
 public:
...
traffic_alpr::PlateWorkItem job(uint64_t track_id) {
...
int main() {
  traffic_alpr::PlateJobQueue queue(2U);
  ...
  try {
    traffic_alpr::PlateJobQueue invalid(0U);
  ...
  traffic_alpr::PlateJobQueue lease_queue(1U);
  ...
  traffic_alpr::EngineBatchProfiles profiles;
  profiles.plate = {1U, 4U, 4U, true};
  traffic_alpr::ProcessingCoordinator coordinator(config, profiles);
  ...
  traffic_alpr::ProcessingCoordinator draining(small_queue_config, profiles);
  std::atomic<size_t> handled{0U};
  traffic_alpr::PlateWorker worker(
      &draining, [&handled](const std::vector<traffic_alpr::PlateWorkItem>& jobs) {
  ...
  traffic_alpr::ProcessingCoordinator failing(config, profiles);
  traffic_alpr::PlateWorker failing_worker(
      &failing, [](const std::vector<traffic_alpr::PlateWorkItem>&) -> size_t {
