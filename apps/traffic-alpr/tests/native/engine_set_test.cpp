...
int main() {
  ...
  const traffic_alpr::RuntimeIdentity runtime{
      "9.1", "13.0", "10.0", "RTX 3060", "8.6"};
  const traffic_alpr::ModelSourceHashes sources{
      {"vehicle", "source-v"}, {"plate", "source-p"}, {"lprnet", "source-l"}};
  traffic_alpr::EngineSet selected;
  std::string error;
  assert(traffic_alpr::select_engine_set(
  ...
  assert(!traffic_alpr::validate_engine_set(
