...
int main() {
  using traffic_alpr::ServiceState;
  assert(std::string(traffic_alpr::service_state_name(ServiceState::kStarting)) ==
         "starting");
  assert(std::string(traffic_alpr::service_state_name(ServiceState::kDegraded)) ==
         "degraded");

  traffic_alpr::ReconnectBackoff backoff(1000U, 30000U);
  ...
  traffic_alpr::PublishQueue queue(2U);
