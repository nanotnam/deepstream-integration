...
int main() {
  ...
  const deepstream_runtime::RuntimeIdentity runtime{
      "9.1", "13.0", "10.0", "RTX 3060", "8.6"};
  const deepstream_runtime::ModelSourceHashes sources{
      {"vehicle", "source-v"}, {"plate", "source-p"}, {"lprnet", "source-l"}};
  deepstream_runtime::EngineSet selected;
  std::string error;
  assert(deepstream_runtime::select_engine_set(
  ...
  assert(!deepstream_runtime::validate_engine_set(
