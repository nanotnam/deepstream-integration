...
#include "deepstream_runtime/gpu_inference_executor.hpp"
 ...
 private:
  ...
  static gboolean on_health_timer(gpointer data) {
    ...
    const bool reconnecting = self->reconnecting_.load() ||
        (self->publisher_ != nullptr && self->publisher_->reconnecting());
    const bool degraded = (self->publisher_ != nullptr &&
                           self->publisher_->degraded()) ||
  ...
  void publish_event(const alpr::AlprEvent& event) {
    ...
    if (publisher_ != nullptr) {
      static_cast<void>(publisher_->publish_event(json));
    } else {
  ...
  void publish_health(const std::string& state) {
    ...
    write_stdout(json);
    if (publisher_ != nullptr) {
      static_cast<void>(publisher_->publish_health(json));
    }
  }
  ...
  std::unique_ptr<KafkaPublisher> publisher_;
